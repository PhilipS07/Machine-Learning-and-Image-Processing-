...
class UseCaseDiagramDetector:
    ...
    def detect_elements(self, image_path):
        ...
        # Save debug image to same directory as output will go
        debug_path = os.path.join(os.path.dirname(image_path) or '.', 'binary_preprocessed.jpg')
        try:
            cv2.imwrite(debug_path, binary)
        except:
            # Continue even if we can't save the debug image
            pass
    ...
    def display_results(self, results):
        ...
        try:
            ...
            print(f"Warning: Could not display results window: {e}")
            print("You may need to run this in a graphical environment")
...
# Main function to run detector
def main():
    import argparse
    import os
    
    # Set up command line arguments
    parser = argparse.ArgumentParser(description='Detect elements in use case diagrams')
    parser.add_argument('--image', '-i', required=False, default='case1.png',
                        help='Path to the input diagram image')
    parser.add_argument('--output', '-o', required=False, default='annotated_diagram.jpg',
                        help='Path to save the annotated output image')
    parser.add_argument('--no-display', action='store_true', 
                        help='Skip displaying the results window')
    parser.add_argument('--tesseract', '-t', required=False,
